...
/*   Updated: 2023/02/23 18:43:11 by lzi-xian         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_init_philo_data(t_philo *philo)
{
	...
	while (i < philo->number_of_philo)
	{
		...
		sem_unlink("left");
		data[i].left = sem_open("left", O_CREAT, 0644, 1);
		if (i > 0)
		{
			data[i].right = &data[i - 1].left;
		}
		if (i + 1 == philo->number_of_philo && i + 1 > 1)
		{
			data[0].right = &data[i].left;
		}
		data[i].vars = philo;
		i++;
	}
...
void	ft_init_philo_cont(t_philo *philo)
{
	...
	sem_unlink("print");
	philo->printing = sem_open("print", O_CREAT, 0644, philo->number_of_philo);
	ft_death_and_finish(philo);
